...
Status UcxCommunicator::reduce(const void* sendbuff, void* recvbuff, size_t len,
    ...
    CUDA_CHECK(cudaMemcpy(rbuffer, sendbuff, len * size,
        ...
        if ((virtual_rank & mask) == 0) {
            if ((virtual_rank & bit) != 0) {
                ...
                if (virtual_dest < m_nranks) {  // valid dest
                    MEGRAY_CHECK(_isend(rbuffer, len * size, actual_dest));
                    MEGRAY_CHECK(_flush());
                    CUDA_CHECK(cudaStreamSynchronize(stream));
                }
            } else {
                ...
                if (virtual_src < m_nranks) {  // valid src
                    MEGRAY_CHECK(_irecv(lbuffer, len * size, actual_src));
                    MEGRAY_CHECK(_flush());
                    MegRay::reduce(lbuffer, rbuffer, rbuffer, len, dtype, op,
                                   stream);
                    CUDA_CHECK(cudaStreamSynchronize(stream));
                }
            }
    ...
    if (m_rank == root) {
        CUDA_CHECK(cudaMemcpy(recvbuff, rbuffer, len * size,
                              cudaMemcpyDeviceToDevice));
    }
