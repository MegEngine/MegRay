...
Status UcxCommunicator::reduce_scatter(const void* sendbuff, void* recvbuff,
    ...
    CUDA_CHECK(cudaMemcpy(rbuffer, sendbuff, recvlen * m_nranks * size,
                          cudaMemcpyDeviceToDevice));

    // pass and add (i-1)-th part from i-th node to i+1 to i+2 finally to
    // (i-1)-th node at last i-th node has the sum of i-th part data
    ...
    CUDA_CHECK(cudaMemcpy(recvbuff, rbuffer + offset, recvlen * size,
                          cudaMemcpyDeviceToDevice));
