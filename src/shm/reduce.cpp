...
Status ShmCommunicator::_shm_reduce(const void* sendbuff, void* recvbuff,
...
Status ShmCommunicator::reduce(const void* sendbuff, void* recvbuff, size_t len,
                               DType dtype, ReduceOp op, uint32_t root,
                               std::shared_ptr<Context> ctx) {
    // only support in single machine
    if (!m_is_single_machine) {
        MEGRAY_THROW("shm reduce only support in single machine now");
    }
    return _shm_reduce(sendbuff, recvbuff, len, dtype, op, root, ctx);
}
